...
import { getBookById } from '@/lib/api/googleBooks'
import type { BookDetail } from '@/types/bookDetail'
import { fetchReviews, postReview, voteReview } from '@/lib/api/reviews'
...
export default function BookDetailPage() {
  ...
  useEffect(() => {
    ...
    fetchReviews(id)
      .then(setReviews)
      .catch((err: unknown) => console.error('Error cargando reseñas', err))
  }, [id])
  ...
  function handleNewReview(data: {
    ...
    postReview(data)
      .then(() => fetchReviews(data.bookId))
      .then(setReviews)
      .catch((err: unknown) => console.error('Error añadiendo reseña', err))
  }
  ...
  function handleVote(
    ...
    voteReview(reviewId, delta)
      .then(() => {
        setReviews(current =>
          current.map(r => (r.id === reviewId ? { ...r, ...delta } : r))
        )
      })
      .catch((err: unknown) => console.error('Error actualizando voto', err))
  }
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 space-y-6">
      ...
      <section>
        ...
        <ReviewForm bookId={id as string} onSubmit={handleNewReview} />
      </section>
    </div>
  )
}
